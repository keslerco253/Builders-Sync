    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40, maxWidth: 680, width: '100%', alignSelf: 'center' }}>
        ...
        <View style={{ alignItems: 'center', marginBottom: 28 }}>
          <View style={[sty.bigAvatar, { backgroundColor: rG(user?.role, C) }]}>
            <Text style={sty.bigAvatarTxt}>{ini(user?.name)}</Text>
          </View>
          <Text style={{ fontSize: 22, fontWeight: '700', color: C.textBold, marginTop: 12 }}>{user?.name}</Text>
          <View style={[sty.rolePill, { backgroundColor: rG(user?.role, C) + '22' }]}>
        ...
        <View style={sty.card}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <Text style={[sty.cardTitle, { marginBottom: 0 }]}>Account Information</Text>
            {!editing && (
            ...
            <View>
              <View style={{ marginBottom: 14 }}>
                ...
                <TextInput value={editEmail} onChangeText={setEditEmail} placeholder="Email address"
                  placeholderTextColor={C.ph} keyboardType="email-address" autoCapitalize="none" style={sty.inp} />
              </View>
              <View style={{ marginBottom: 14 }}>
                <Text style={sty.lbl}>COMPANY</Text>
                <TextInput value={editCompany} onChangeText={setEditCompany} placeholder="Company name"
                  placeholderTextColor={C.ph} style={sty.inp} />
              </View>
              <View style={{ marginBottom: 14 }}>
                <Text style={sty.lbl}>PHONE</Text>
                <TextInput value={editPhone} onChangeText={setEditPhone} placeholder="Phone number"
                  placeholderTextColor={C.ph} keyboardType="phone-pad" style={sty.inp} />
              </View>
              {readOnlyRows.map(([label, val], i) => (
                <View key={label}>
                  <View style={sty.infoRow}>
                    <Text style={sty.infoLbl}>{label.toUpperCase()}</Text>
                    <Text style={sty.infoVal}>{val}</Text>
                  </View>
                  {i < readOnlyRows.length - 1 && <View style={sty.divider} />}
                </View>
              ))}
              <View style={{ flexDirection: 'row', gap: 10, marginTop: 16 }}>
                ...
                <TouchableOpacity onPress={saveProfile} disabled={saving}
                  style={[sty.submitBtn, { flex: 1, marginTop: 0 }, saving && { backgroundColor: C.dm }]}
                  activeOpacity={0.7}>
            ...
            <>
              {[
                ...
                <View key={label}>
                  ...
                  {i < arr.length - 1 && <View style={sty.divider} />}
                </View>
              ))}
            </>
          )}
        ...
        {/* Actions */}
        <TouchableOpacity onPress={() => setShowPwModal(true)} style={[sty.card, { flexDirection: 'row', alignItems: 'center', gap: 12 }]}>
        ...
        <TouchableOpacity onPress={handleSignOut} style={[sty.card, { flexDirection: 'row', alignItems: 'center', gap: 12, borderColor: 'rgba(239,68,68,0.15)' }]}>
        ...
        <View style={{ alignItems: 'center', marginTop: 30 }}>
          <View style={[sty.logoBox]}><Feather name="hexagon" size={14} color={C.textBold} /></View>
        ...
        <View style={sty.modalBg}>
          <View style={sty.modalContent}>
            <View style={sty.modalHead}>
              <Text style={sty.modalTitle}>Change Password</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={{ marginBottom: 14 }}>
                <Text style={sty.lbl}>CURRENT PASSWORD</Text>
                <TextInput value={cur} onChangeText={v => { setCur(v); setErr(''); }} placeholder="Current password"
                  placeholderTextColor={C.ph} secureTextEntry style={sty.inp} />
              </View>
              <View style={{ marginBottom: 14 }}>
                <Text style={sty.lbl}>NEW PASSWORD (MIN 8)</Text>
                <TextInput value={np} onChangeText={v => { setNp(v); setErr(''); }} placeholder="New password"
                  placeholderTextColor={C.ph} secureTextEntry style={sty.inp} />
              </View>
              <View style={{ marginBottom: 14 }}>
                <Text style={sty.lbl}>CONFIRM</Text>
              ...
              <TouchableOpacity onPress={submit} disabled={!cur || !np || !conf || loading}
                style={[sty.submitBtn, (!cur || !np || !conf || loading) && { backgroundColor: C.dm }]}>
