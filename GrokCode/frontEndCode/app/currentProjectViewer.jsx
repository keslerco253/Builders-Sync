...
// MAIN COMPONENT
// ============================================================
const CurrentProjectViewer = ({ embedded, project: projectProp, clientView, onClientViewToggle, subView, onSubViewToggle, activeTab, activeSub, onTabChange, onSubChange, onProjectUpdate, onProjectDeleted, scheduleVersion, onScheduleChange, syncRef, calYear, calMonth, onMonthChange, subdivisions = [] }) => {
  const C = React.useContext(ThemeContext);
  const s = React.useMemo(() => getStyles(C), [C]);
  ...
  const isB = (clientView || subView) ? false : user?.role === 'builder';
  const isC = clientView ? true : subView ? false : user?.role === 'customer';
  const isCon = clientView ? false : subView ? true : user?.role === 'contractor';
      ...
      {subView && (
        <View style={{
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={{ fontSize: 14, color: C.dm }}>— Viewing as subcontractor</Text>
          </View>
          {onSubViewToggle && (
            <TouchableOpacity
              onPress={onSubViewToggle}
